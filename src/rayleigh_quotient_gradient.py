...
from .rayleigh_quotient import rayleigh_quotient, rayleigh_quotient_naive
...
def rayleigh_quotient_gradient(A: np.ndarray, x: np.ndarray) -> np.ndarray:
    """
    ...
    rq = x @ t  # rq = rayleigh_quotient(A, x)
    return k * (t - rq * x)


@verify_not_modified(VERIFY_NOT_MODIFIED)
def rayleigh_quotient_gradient_naive(A: np.ndarray, X: np.ndarray) -> np.ndarray:
    """
    ...
    rq_naive = rayleigh_quotient_naive(A, X)
